...
export const SecondaryDonut: FC = () => {
  ...
  return (
    <>
      <Main>
        <Header>
          {isActive === 'Affinities' ? (
            <HeaderItemsAcive>Affinities</HeaderItemsAcive>
          ) : (
            <HeaderItems onClick={() => changeActiveElement('Affinities')}>
              Affinities
            </HeaderItems>
          )}
          {isActive === 'Nonprofits' ? (
            <HeaderItemsAcive>Nonprofits</HeaderItemsAcive>
          ) : (
            <HeaderItems onClick={() => changeActiveElement('Nonprofits')}>
              Nonprofits
            </HeaderItems>
          )}
          {isActive === 'Purchasing' ? (
            <HeaderItemsAcive>Purchasing</HeaderItemsAcive>
          ) : (
            <HeaderItems onClick={() => changeActiveElement('Purchasing')}>
              Purchasing
            </HeaderItems>
          )}
          {isActive === 'Brands' ? (
            <HeaderItemsAcive>Brands</HeaderItemsAcive>
          ) : (
            <HeaderItems onClick={() => changeActiveElement('Brands')}>
              Brands
            </HeaderItems>
          )}
          {isActive === 'Interests' ? (
            <HeaderItemsAcive>Interests</HeaderItemsAcive>
          ) : (
            <HeaderItems onClick={() => changeActiveElement('Interests')}>
              Interests
            </HeaderItems>
          )}
        </Header>
      ...
      <Footer>
        {getCurrentState()?.options.labels.map((item: any, index: number) => (
          <FooterItem
            key={index}
            style={{
              backgroundColor: `${getCurrentState()?.options.colors[index]}`,
              color: `${getCurrentState()?.options?.fontColors[index]}`,
            }}
          >
